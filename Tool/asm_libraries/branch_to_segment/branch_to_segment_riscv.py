...
class BranchToSegment_riscv(BranchToSegmentBase):
    def __enter__(self):
        ...
        AsmLogger.print_asm_line(f"jal ra, {self.code_label}")
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        """
        AsmLogger.print_comment_line(f"Return to the previous code segment {self.prev_code_block.name} using the address in `ra` (similar to `ret` in x86)")
        AsmLogger.print_asm_line(f"jr ra")
    ...
    def one_way_branch(self):
        ...
        '''
        AsmLogger.print_comment_line(f"using 'jal' with x0 to branch 'one-way' (similar to jmp) to code segment {self.code_block.name}")
        AsmLogger.print_asm_line(f"jal x0, {self.code_label}   # Unconditional jump to target_label")
        switch_code(self.code_block)
        AsmLogger.print_asm_line(f"{self.code_label}:")
