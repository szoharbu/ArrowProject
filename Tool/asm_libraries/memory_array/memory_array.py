...
class MemoryArray:
    def __init__(self, array_name, elements, element_size=4):
        ...
        if Configuration.Architecture.x86:
            AsmLogger.print_asm_line(f"lea {tmp_reg}, [{self.array_block.unique_label}]",comment=comment1)
            AsmLogger.print_asm_line(f"mov [{self.offset_mem.unique_label}], {tmp_reg}",comment=comment2)
        elif Configuration.Architecture.arm:
            AsmLogger.print_asm_line(f"ldr {tmp_reg}, ={self.array_block.unique_label}",comment=comment1)
            AsmLogger.print_asm_line(f"str {tmp_reg}, [{self.offset_mem.unique_label}]",comment=comment2)
        elif Configuration.Architecture.riscv:
            AsmLogger.print_asm_line(f"la {tmp_reg}, {self.array_block.unique_label}",comment=comment1)
    ...
    def load_element(self, output_register)->None:
        ...
        if Configuration.Architecture.x86:
            AsmLogger.print_asm_line(f"mov {tmp_reg}, [{self.offset_mem.unique_label}]",comment=comment1)
            AsmLogger.print_asm_line(f"mov {output_register}, [{tmp_reg}]",comment=comment2)
        elif Configuration.Architecture.arm:
            AsmLogger.print_asm_line(f"ldr {tmp_reg}, [{self.offset_mem.unique_label}]",comment=comment1)
            AsmLogger.print_asm_line(f"ldr {output_register}, [{tmp_reg}]",comment=comment2)
        elif Configuration.Architecture.riscv:
            AsmLogger.print_asm_line(f"lw {tmp_reg}, {self.offset_mem.unique_label}",comment=comment1)
            AsmLogger.print_asm_line(f"lw {output_register}, 0({tmp_reg})",comment=comment2)
        else:
            raise ValueError(f"Unsupported architecture")
    ...
    def store_element(self, output_register)->None:
        ...
        if Configuration.Architecture.x86:
            AsmLogger.print_asm_line(f"mov {tmp_reg}, [{self.offset_mem.unique_label}]",comment=comment1)
            AsmLogger.print_asm_line(f"mov [{tmp_reg}], {output_register}",comment=comment2)
        elif Configuration.Architecture.arm:
            AsmLogger.print_asm_line(f"ldr {tmp_reg}, [{self.offset_mem.unique_label}]",comment=comment1)
            AsmLogger.print_asm_line(f"str {output_register}, [{tmp_reg}]",comment=comment2)
        elif Configuration.Architecture.riscv:
            AsmLogger.print_asm_line(f"lw {tmp_reg}, {self.offset_mem.unique_label}",comment=comment1)
            AsmLogger.print_asm_line(f"sw {output_register}, 0({tmp_reg})",comment=comment2)
        else:
            raise ValueError(f"Unsupported architecture")
    ...
    def next_element(self, num_elements)->None:
        ...
        AsmLogger.print_comment_line(f"Increment the address in {self.offset_mem.name} by {self.element_size} (element size)")
        if Configuration.Architecture.x86:
